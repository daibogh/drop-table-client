...
import { SpaceProps, propsToSpace } from '../base/utils/SpaceUtil'
...
export const SelectBaseField = <TOption extends object | string | number>({
  ...
  const filter = useMemo(() => {
    ...
    if (options) {
      options.forEach((option, key) => {
        if (getLabel(option).toLowerCase().includes(inputValue.toLowerCase()))
  ...
  const alphabeticalSort = useCallback((a: string, b: string) => {
    const x = a.trim(),
      y = b.trim()
  ...
  const optionsBoxes = useMemo(() => {
    if (!multiselect) {
      ...
      if (alphabeticalOrder)
        result = result.sort((a, b) => alphabeticalSort(a, b))
      result =
        isCell && val.length > maxBoxes ? result.splice(0, maxBoxes) : result
      return result.map((x) => {
        ...
        return (
          <div
            ...
            onDrop={() => {
              if (dnd !== undefined && dnd.from !== x)
                onDropCallback(dnd.from, dnd.to)
            }}
            onClick={(e) => {
              e.stopPropagation()
            }}
            ...
            className={classNames(
              'box',
              dnd?.to === x ? 'box-drag-over' : 'box-regular'
            )}
          >
            {option ? (
              getContent ? (
                ...
                <Line className="box-label">
                  {onlyKeys ? x : getLabel(option)}
                </Line>
              )
            ) : (
              ''
            ...
            <div
              className={classNames('icon-display', { 'icon-hover': !disable })}
            >
              <Icon
                name="times"
                className="icon"
  ...
  const countString =
    multiselect && isCell && value.length > maxBoxes
      ? `and ${value.length - maxBoxes} more`
      : undefined
    ...
    >
      ...
      >
        <Line
          ...
          alignItems="center"
          className={isCell ? 'content cell' : 'content'}
        >
          <Line
            className={noWrap ? 'word-hidden' : undefined}
            wrap={!(isCell && multiselect)}
            alignItems="baseline"
          >
            {isCell && multiselect ? (
              <>
            ...
            {admitRemove && !multiselect && (
              <div
                ...
                style={
                  isCell
                    ? { borderRight: 'none' }
                    : { borderRight: 'solid $light-grey 2px' }
                }
                onClick={
                  disable
                    ? () => {}
              ...
              <div
                ...
                onClick={() => {
                  if (!disable)
                    value?.length != 0
                      ? onChange([])
                      : onChange(Array.from(options).map(([key]) => key))
                }}
              >
                {
                  <Icon
                    name={value?.length != 0 ? 'times' : 'check'}
                    className="icon"
                  ></Icon>
                }
              </div>
        ...
        {show && (
          <div className="dropdown-container">
            <div
              ref={popper}
              className="dropdown"
              onClick={!multiselect ? deactivate : undefined}
            >
              {showSearch && (
                <Line
